...
class Pipeline(Node):
    ...
    def _accumulate(self, start, stop):
        ...
        if self._dimensions == 2:
            ...
            if self._X_train is not None:
                self._X_train_mean = np.mean(self._X_train, axis=0)
        ...
        if self._dimensions == 3:
            ...
            # Calculate mean
            if self._X_train is not None:
                self._X_train_mean = np.mean(self._X_train, axis=(0, 2))
    ...
    def _send(self):
        ...
        if self._out is not None:
            # Calculate mean of X_train if available
            if hasattr(self, '_X_train_mean'):
                if self._X_train_mean is not None:
                    # Create DataFrame with appropriate index
                    mean_data = pd.DataFrame({'mean': self._X_train_mean}, index=self._X_train_indices)
                    event_meta = {"event_type": "mean_X_train"}
                    if self.o_events.ready():
                        self.o_events.data = pd.concat([self.o_events.data, mean_data])
                    else:
                        self.o_events.data = mean_data
                    self.o_events.meta = event_meta
            
            # Send data or predictions
