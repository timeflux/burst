...
if not os.path.exists(os.path.join("graphs", os.environ["DEVICE"]+".yaml")):
	error_handling("DEVICE", f"Could not find graph file {os.environ['DEVICE']}.yaml in the graph folder", "graphs/dummy")
...
if os.environ["CALIBRATION_LAYOUT"] not in ["single", "simple", "grid", "keyboard"]:
	error_handling("CALIBRATION_LAYOUT", f"CALIBRATION_LAYOUT must be one of 'single', 'simple', 'grid' or 'keyboard'", "simple")
...
if not os.path.exists(os.environ["CODE_FILE"]):
	error_handling("CODE_FILE", f"The code file path {os.environ['CODE_FILE']} does not exist", DEFAULT_CODE_FILE)
...
if not os.path.exists(DEFAULT_CODE_FILE):
	error_handling("CODE_FILE", f"The example file for code {os.environ['CODE_FILE']} does not exist", None, True)
...
if len(bad_codes) > 0:
	error_handling("CALIBRATION_CODES", f"The codes {bad_codes} are not composed soloely of 0s and 1s", None, True)
...
if not check_int("NUMBER_OF_CLASSES") or int(os.environ["NUMBER_OF_CLASSES"]) <= 0:
	error_handling("NUMBER_OF_CLASSES", f"NUMBER_OF_CLASSES must be a strictly positive integer", 2)
if int(os.environ["NUMBER_OF_CLASSES"]) > len(codes):
	error_handling("NUMBER_OF_CLASSES", f"Not enough codes {len(codes)} for the specified number of classes", len(codes))

codes = ' '.join([''.join(list(map(str, c))) for c in codes[:int(os.environ["NUMBER_OF_CLASSES"])]])
...
if not os.environ["CHANNELS"]:
	error_handling("CHANNELS", "CHANNELS must a list of coma separated  name of electrodes (spaces are trimmed)", None, True)
...
if os.environ["REREF_CHANNEL"] is None:
	error_handling("REREF_CHANNEL", "REREF_CHANNEL not set meaning an average rereferencing will be used", "")
elif not os.environ["REREF_CHANNEL"] in chans:
	error_handling("REREF_CHANNEL", "REREF_CHANNEL is set but the electrode was not found in the list of electrodes", None, True)

# Check tha tit is in gabor/ricker
os.environ["STIM_TYPE"] = os.getenv("STIM_TYPE")
if os.environ["STIM_TYPE"] not in ["ricker", "gabor"]:
	error_handling("STIM_TYPE", f"STIM_TYPE must be one of 'ricker' or 'gabor'", "ricker")

# Check float in range [0..1]
os.environ["STIM_DEPTH"] = os.getenv("STIM_DEPTH")
if not check_float("STIM_DEPTH") or float(os.environ["STIM_DEPTH"]) <= 0 or float(os.environ["STIM_DEPTH"]) > 1:
...
if not re.search(color_regex, os.environ["UI_BACKGROUND"]):
	error_handling("UI_BACKGROUND", "UI_BACKGROUND must be an HTML color (without the #) of the form [0-9A-Za-z]\{6\}", "202020")
os.environ["UI_TEXT_COLOR"] = os.getenv("UI_TEXT_COLOR")
if not re.search(color_regex, os.environ["UI_TEXT_COLOR"]):
	error_handling("UI_TEXT_COLOR", "UI_TEXT_COLOR must be an HTML color (without the #) of the form [0-9A-Za-z]\{6\}", "FFFFFF")
os.environ["UI_CROSS_COLOR"] = os.getenv("UI_CROSS_COLOR")
if not re.search(color_regex, os.environ["UI_CROSS_COLOR"]):
	error_handling("UI_CROSS_COLOR", "UI_CROSS_COLOR must be an HTML color (without the #) of the form [0-9A-Za-z]\{6\}", "FFFFFF")
os.environ["UI_TARGET_OFF_COLOR"] = os.getenv("UI_TARGET_OFF_COLOR")
if not re.search(color_regex, os.environ["UI_TARGET_OFF_COLOR"]):
	error_handling("UI_TARGET_OFF_COLOR", "UI_TARGET_OFF_COLOR must be an HTML color (without the #) of the form [0-9A-Za-z]\{6\}", "797979")
os.environ["UI_TARGET_ON_COLOR"] = os.getenv("UI_TARGET_ON_COLOR")
if not re.search(color_regex, os.environ["UI_TARGET_ON_COLOR"]):
	error_handling("UI_TARGET_ON_COLOR", "UI_TARGET_ON_COLOR must be an HTML color (without the #) of the form [0-9A-Za-z]\{6\}", "FFFFFF")
...
if not re.search(color_regex, os.environ["UI_TARGET_BORDER_COLOR"]):
	error_handling("UI_TARGET_BORDER_COLOR", "UI_TARGET_BORDER_COLOR must be an HTML color (without the #) of the form [0-9A-Za-z]\{6\}", "000000")
os.environ["UI_TARGET_CUE_COLOR"] = os.getenv("UI_TARGET_CUE_COLOR")
if not re.search(color_regex, os.environ["UI_TARGET_CUE_COLOR"]):
	error_handling("UI_TARGET_CUE_COLOR", "UI_TARGET_CUE_COLOR must be an HTML color (without the #) of the form [0-9A-Za-z]\{6\}", "0000FF")
...
if not re.search(color_regex, os.environ["UI_TARGET_SUCCESS_COLOR"]):
	error_handling("UI_TARGET_SUCCESS_COLOR", "UI_TARGET_SUCCESS_COLOR must be an HTML color (without the #) of the form [0-9A-Za-z]\{6\}", "00FF00")
...
if not re.search(color_regex, os.environ["UI_TARGET_FAILURE_COLOR"]):
	error_handling("UI_TARGET_FAILURE_COLOR", "UI_TARGET_FAILURE_COLOR must be an HTML color (without the #) of the form [0-9A-Za-z]\{6\}", "FF0000")
...
if not re.search(color_regex, os.environ["UI_TARGET_LOCK_COLOR"]):
	error_handling("UI_TARGET_LOCK_COLOR", "UI_TARGET_LOCK_COLOR must be an HTML color (without the #) of the form [0-9A-Za-z]\{6\}", "0000FF")
...
if not check_int("CALIBRATION_BLOCKS") or int(os.environ["CALIBRATION_BLOCKS"]) < 0:
	error_handling("CALIBRATION_BLOCKS", "CALIBRATION_BLOCKS must be a positive integer (can be 0 to skip calibration)", 5)
os.environ["CALIBRATION_STIM_REP"] = os.getenv("CALIBRATION_STIM_REP")
if not check_int("CALIBRATION_STIM_REP") or int(os.environ["CALIBRATION_STIM_REP"]) <= 0:
	error_handling("CALIBRATION_STIM_REP", "CALIBRATION_STIM_REP must be a strictly positive integer, in ms", 1)
os.environ["CALIBRATION_DURATION_REST"] = os.getenv("CALIBRATION_DURATION_REST")
if not check_int("CALIBRATION_DURATION_REST") or int(os.environ["CALIBRATION_DURATION_REST"]) <= 0:
	error_handling("CALIBRATION_DURATION_REST", "CALIBRATION_DURATION_REST must be a strictly positive integer, in ms", 2000)
os.environ["CALIBRATION_DURATION_CUE_ON"] = os.getenv("CALIBRATION_DURATION_CUE_ON")
if not check_int("CALIBRATION_DURATION_CUE_ON") or int(os.environ["CALIBRATION_DURATION_CUE_ON"]) <= 0:
	error_handling("CALIBRATION_DURATION_CUE_ON", "CALIBRATION_DURATION_CUE_ON must be a strictly positive integer, in ms", 1500)
os.environ["CALIBRATION_DURATION_CUE_OFF"] = os.getenv("CALIBRATION_DURATION_CUE_OFF")
if not check_int("CALIBRATION_DURATION_CUE_OFF") or int(os.environ["CALIBRATION_DURATION_CUE_OFF"]) <= 0:
	error_handling("CALIBRATION_DURATION_CUE_OFF", "CALIBRATION_DURATION_CUE_OFF must be a strictly positive integer, in ms", 500)
...
if not check_int("TASK_DURATION_REST") or int(os.environ["TASK_DURATION_REST"]) <= 0:
	error_handling("TASK_DURATION_REST", "TASK_DURATION_REST must be a strictly positive integer, in ms", 2000)
os.environ["TASK_DURATION_LOCK_ON"] = os.getenv("TASK_DURATION_LOCK_ON")
if not check_int("TASK_DURATION_LOCK_ON") or int(os.environ["TASK_DURATION_LOCK_ON"]) <= 0:
	error_handling("TASK_DURATION_LOCK_ON", "TASK_DURATION_LOCK_ON must be a strictly positive integer, in ms", 1500)
os.environ["TASK_DURATION_LOCK_OFF"] = os.getenv("TASK_DURATION_LOCK_OFF")
if not check_int("TASK_DURATION_LOCK_OFF") or int(os.environ["TASK_DURATION_LOCK_OFF"]) <= 0:
	error_handling("TASK_DURATION_LOCK_OFF", "TASK_DURATION_LOCK_OFF must be a strictly positive integer, in ms", 500)

#  Check boolean
os.environ["CUED_TASK_ENABLE"] = os.getenv("CUED_TASK_ENABLE")
if not check_boolean("CUED_TASK_ENABLE"):
	error_handling("CUED_TASK_ENABLE", "CUED_TASK_ENABLE must be a boolean (true/false)", True)
os.environ["PINPAD_ENABLE_TASK"] = os.getenv("PINPAD_ENABLE_TASK")
if not check_boolean("PINPAD_ENABLE_TASK"):
	error_handling("PINPAD_ENABLE_TASK", "PINPAD_ENABLE_TASK must be a boolean (true/false)", True)
os.environ["PINPAD_CUE_TARGET"] = os.getenv("PINPAD_CUE_TARGET")
if not check_boolean("PINPAD_CUE_TARGET"):
	error_handling("PINPAD_CUE_TARGET", "PINPAD_CUE_TARGET must be a boolean (true/false)", False)
os.environ["PINPAD_CUE_FEEDBACK"] = os.getenv("PINPAD_CUE_FEEDBACK")
if not check_boolean("PINPAD_CUE_FEEDBACK"):
	error_handling("PINPAD_CUE_FEEDBACK", "PINPAD_CUE_FEEDBACK must be a boolean (true/false)", True)

# Check integer positive non zero
os.environ["CUED_TASK_TARGET_REP"] = os.getenv("CUED_TASK_TARGET_REP")
if not check_int("CUED_TASK_TARGET_REP") or int(os.environ["CUED_TASK_TARGET_REP"]) <= 0:
	error_handling("CUED_TASK_TARGET_REP", "CUED_TASK_TARGET_REP must be a strictly positive integer", 20)
...
if not check_int("PINPAD_SEQUENCES") or int(os.environ["PINPAD_SEQUENCES"]) <= 0:
	error_handling("PINPAD_SEQUENCES", "PINPAD_SEQUENCES must be a strictly positive integer", 5)
